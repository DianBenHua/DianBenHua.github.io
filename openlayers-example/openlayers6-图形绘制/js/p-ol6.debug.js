...
P.PlotTypes = {
    ...
    RECTANGLE: "rectangle", 
	STAR: "star", 
	REGULAR_TRIANGLE: "regulartriangle",
	SQUARE: "square",
...
/* 文字 */
P.Plot.Text = function(points){
    goog.base(this, [0,0]);
    this.type = P.PlotTypes.TEXT;
    this.fixPointCount = 1;
    this.setPoints(points);
}
goog.inherits(P.Plot.Text, ol.geom.Point);
goog.mixin(P.Plot.Text.prototype, P.Plot.prototype);
P.Plot.Text.prototype.generate = function(){
    var pnt = this.points[0];
    this.setCoordinates(pnt);
};
...
/* 五星 */
P.Plot.Star = function(points){
    goog.base(this, []);
    this.type = P.PlotTypes.STAR;
    this.fixPointCount = 2;
    this.setPoints(points);
};
goog.inherits(P.Plot.Star, ol.geom.Polygon);
goog.mixin(P.Plot.Star.prototype, P.Plot.prototype);
P.Plot.Star.prototype.generate = function(){
    var count = this.getPointCount();
    if(count<2) {
        return;
    }else{
		...
		for (var i = 0; i < numPoints; ++i) {
			var angle = rotation + i * 2 * Math.PI / numPoints - 0.63;
...
/* 正三角形 */
P.Plot.RegularTriangle = function(points){
   goog.base(this, []);
    this.type = P.PlotTypes.REGULAR_TRIANGLE;
    this.fixPointCount = 2;
	// 正三角形的边个数
	this.numSides = 3;
	// 辅助点位置偏差(双正N变形是不用调的，只有单正N变形才会调，一般都在零点几)
	this.helperPositionOffset = 1.05;
    this.setPoints(points);
};
goog.inherits(P.Plot.RegularTriangle, ol.geom.Polygon);
goog.mixin(P.Plot.RegularTriangle.prototype, P.Plot.prototype);
P.Plot.RegularTriangle.prototype.generate = function(){
    var count = this.getPointCount();
    if(count<2) {
        return;
    }else{
		...
		for (var i = 0; i < this.numSides; ++i) {
			var angle = rotation + i * 2 * Math.PI / this.numSides - this.helperPositionOffset;
...
/* 正四边形 */
P.Plot.Square = function(points){
    goog.base(this, []);
    this.type = P.PlotTypes.SQUARE;
    this.fixPointCount = 2;
	// 正四边形的边个数
	this.numSides = 4;
	// 辅助点位置偏差
	this.helperPositionOffset = 0;
    this.setPoints(points);
};
// 继承正三角形
goog.inherits(P.Plot.Square, P.Plot.RegularTriangle);

/* 正五边形 */
P.Plot.RegularPentagon  = function(points){
    goog.base(this, []);
    this.type = P.PlotTypes.REGULAR_PENTAGON;
    this.fixPointCount = 2;
	this.numSides = 5;
	this.helperPositionOffset = 0.63;
    this.setPoints(points);
};
// 继承正三角形
goog.inherits(P.Plot.RegularPentagon, P.Plot.RegularTriangle);
...
P.PlotFactory.createPlot = function(type, points){
    switch(type){
        case P.PlotTypes.ARC:// 弧线
            return new P.Plot.Arc(points);
        case P.PlotTypes.ELLIPSE:// 椭圆
            return new P.Plot.Ellipse(points);
        case P.PlotTypes.CURVE:// 曲线
            return new P.Plot.Curve(points);
        case P.PlotTypes.CLOSED_CURVE:// 曲线面
            return new P.Plot.ClosedCurve(points);
        case P.PlotTypes.LUNE:// 弓形
            return new P.Plot.Lune(points);
        case P.PlotTypes.SECTOR:// 扇形
            return new P.Plot.Sector(points);
        case P.PlotTypes.GATHERING_PLACE:// 聚集地
            return new P.Plot.GatheringPlace(points);
        case P.PlotTypes.STRAIGHT_ARROW:// 直箭头
            return new P.Plot.StraightArrow(points);
        case P.PlotTypes.ASSAULT_DIRECTION:// 突击方向
            return new P.Plot.AssaultDirection(points);
        case P.PlotTypes.ATTACK_ARROW:// 进攻方向
            return new P.Plot.AttackArrow(points);
        case P.PlotTypes.FINE_ARROW:// 细直箭头
            return new P.Plot.FineArrow(points);
        case P.PlotTypes.CIRCLE:// 圆
            return new P.Plot.Circle(points);
        case P.PlotTypes.DOUBLE_ARROW:// 钳击
            return new P.Plot.DoubleArrow(points);
        case P.PlotTypes.TAILED_ATTACK_ARROW:// 进攻方向（尾）
            return new P.Plot.TailedAttackArrow(points);
        case P.PlotTypes.SQUAD_COMBAT:// 分队战斗行动
            return new P.Plot.SquadCombat(points);
        case P.PlotTypes.TAILED_SQUAD_COMBAT:// 分队战斗行动（尾）
            return new P.Plot.TailedSquadCombat(points);
        case P.PlotTypes.FREEHAND_LINE:// 自由线
            return new P.Plot.FreehandLine(points);
        case P.PlotTypes.FREEHAND_POLYGON:// 自由面
            return new P.Plot.FreehandPolygon(points);
        case P.PlotTypes.POLYGON:// 多边形
            return new P.Plot.Polygon(points);
        case P.PlotTypes.MARKER:// 点
            return new P.Plot.Marker(points);
		case P.PlotTypes.TEXT:// 文字
            return new P.Plot.Text(points);
        case P.PlotTypes.RECTANGLE:// 矩形
            return new P.Plot.Rectangle(points);
		case P.PlotTypes.STAR:// 五星
            return new P.Plot.Star(points);
		case P.PlotTypes.REGULAR_TRIANGLE:// 正三角形
			return new P.Plot.RegularTriangle(points);
		case P.PlotTypes.SQUARE:// 正四边形
			return new P.Plot.Square(points);
		case P.PlotTypes.REGULAR_PENTAGON:// 正五边形
			return new P.Plot.RegularPentagon(points);
        case P.PlotTypes.POLYLINE:// 折线
...
P.PlotDraw.prototype.mapFirstClickHandler = function (e) {
	var _self = P.PlotDraw._self;
	
    _self.points.push(e.coordinate);
    _self.plot = P.PlotFactory.createPlot(_self.plotType, _self.points, _self.plotParams);
    _self.feature = new ol.Feature(_self.plot);
    _self.featureSource.addFeature(_self.feature);
    _self.map.un("click", _self.mapFirstClickHandler, _self);
    //
    if (_self.plot.fixPointCount == _self.plot.getPointCount()) {
        _self.mapDoubleClickHandler(e);
        return;
    }
    //
    _self.map.on("click", _self.mapNextClickHandler, _self);
    if(!_self.plot.freehand){
        _self.map.on("dblclick", _self.mapDoubleClickHandler, _self);
    }
    goog.events.listen(_self.mapViewport, P.Event.EventType.MOUSEMOVE,
        _self.mapMouseMoveHandler, false, _self);
};
...
P.PlotDraw.prototype.mapNextClickHandler = function (e) {
    ...
    if(!_self.plot.freehand){
        if (P.PlotUtils.distance(e.coordinate, _self.points[_self.points.length - 1]) < P.Constants.ZERO_TOLERANCE)
            return;
    }
    _self.points.push(e.coordinate);
    _self.plot.setPoints(_self.points);
    if (_self.plot.fixPointCount == _self.plot.getPointCount()) {
        _self.mapDoubleClickHandler(e);
        return;
    }
    if(_self.plot && _self.plot.freehand){
        _self.mapDoubleClickHandler(e);
    }
};
...
P.PlotDraw.prototype.mapDoubleClickHandler = function (e) {
    ...
    _self.disconnectEventHandlers();
    _self.plot.finishDrawing();
    e.preventDefault();
    _self.drawEnd();
};
...
P.PlotEdit.prototype.plotMouseOverOutHandler = function(e){
	var _self = P.PlotEdit._self;
	
    var feature = map.forEachFeatureAtPixel(e.pixel, function (feature, layer) {
        return feature;
    });
    if(feature && feature == _self.activePlot){
        if(!_self.mouseOver){
            _self.mouseOver = true;
            _self.map.getViewport().style.cursor = 'move';
            _self.map.on('pointerdown', _self.plotMouseDownHandler, _self);
        }
    }else{
        if(_self.mouseOver){
            _self.mouseOver = false;
            _self.map.getViewport().style.cursor = 'default';
            _self.map.un('pointerdown', _self.plotMouseDownHandler, _self);
        }
    }
};
...
P.PlotEdit.prototype.plotMouseDownHandler = function(e){
    ...
    _self.ghostControlPoints = _self.getControlPoints();
    _self.startPoint = e.coordinate;
    _self.disableMapDragPan();
    _self.map.on('pointerup', _self.plotMouseUpHandler, _self);
    _self.map.on('pointerdrag', _self.plotMouseMoveHandler, _self);
};
...
P.PlotEdit.prototype.plotMouseMoveHandler = function(e){
    ...
    var dx = point[0] - _self.startPoint[0];
    var dy = point[1] - _self.startPoint[1];
    var newPoints = [];
    for(var i=0; i<_self.ghostControlPoints.length; i++){
        var p = _self.ghostControlPoints[i];
        var coordinate = [p[0] + dx, p[1] + dy];
        newPoints.push(coordinate);
        var id = _self.Constants.HELPER_CONTROL_POINT_DIV + '-' + i;
        var overlay = _self.map.getOverlayById(id);
        overlay.setPosition(coordinate);
        overlay.setPositioning('center-center');
    }
    var plot = _self.activePlot.getGeometry();
...
P.PlotEdit.prototype.plotMouseUpHandler = function(e){
    ...
    _self.enableMapDragPan();
    _self.map.un('pointerup', _self.plotMouseUpHandler, _self);
    _self.map.un('pointerdrag', _self.plotMouseMoveHandler, _self);
};
