...
(function (global, factory) {
    ...
    (global = global || self, factory(global.OlWind = {}, global.ol.layer, global.ol.proj, global.ol.renderer.canvas.ImageLayer, global.ol.transform, global.ol.extent));
}(this, (function (exports, layer, proj, CanvasLayerRenderer, transform, extent) { 'use strict';

    CanvasLayerRenderer = CanvasLayerRenderer && CanvasLayerRenderer.hasOwnProperty('default') ? CanvasLayerRenderer['default'] : CanvasLayerRenderer;
    ...
    ***************************************************************************** */
    ...
    var Render = /** @class */ (function () {
        function Render() {
            this.executors = {};
            this.renderedTransform_ = transform.create();
        }
        Render.prototype.execute = function (context, index, frameState, transform, renderedTransform, opt, data) {
        ...
        Render.prototype.setOptions = function (options) {
            var _this = this;
        ...
        Render.prototype.repeatWorld = function (coordinates, pixelCoordinates, transform$1) {
            var pixel;
            if (pixelCoordinates && equals(transform$1, this.renderedTransform_)) {
                ...
                pixel = transform2D(coordinates, 0, coordinates.length, 2, transform$1, pixelCoordinates);
                transform.setFromArray(this.renderedTransform_, transform$1);
            }
            return pixel;
        };
        Render.prototype.getPixelFromCoordinateInternal = function (frameState, transform$1, coordinate) {
            var viewState = frameState.viewState;
            var pixelRatio = frameState.pixelRatio;
            var point = proj.transform(coordinate, 'EPSG:4326', viewState.projection);
            var viewCoordinate = proj.fromUserCoordinate(point, viewState.projection);
            if (!frameState) {
                return null;
            }
            else {
                var pixelCoordinates = transform.apply(frameState.coordinateToPixelTransform, viewCoordinate.slice(0, 2));
        ...
        Render.prototype.getCoordinateFromPixel = function (frameState, pixel) {
            var viewState = frameState.viewState;
            ...
            if (!frameState) {
                ...
                var viewCoordinate = transform.apply(frameState.pixelToCoordinateTransform, pixel.slice(0, 2));
                // const pixel = this.repeatWorld(viewCoordinate.slice(0, 2), pixelCoordinates, transform);
                var coordinate = proj.toUserCoordinate(viewCoordinate, viewState.projection);
                var point = proj.transform(coordinate, viewState.projection, 'EPSG:4326');
                return [
                    point[0],
                    point[1]
                ];
        ...
        Render.prototype.intersectsCoordinate = function (frameState, coordinate) {
            var viewState = frameState.viewState;
            var point = proj.transform(coordinate, 'EPSG:4326', viewState.projection);
            var viewCoordinate = proj.fromUserCoordinate(point, viewState.projection);
    ...
    var WindLayerRender = /** @class */ (function (_super) {
        ...
        WindLayerRender.prototype.useContainer = function (target, transform, opacity) {
            // @ts-ignore
            _super.prototype.useContainer.call(this, null, transform, opacity);
        };
        WindLayerRender.prototype.prepareFrame = function (frameState) {
        ...
        WindLayerRender.prototype.renderFrame = function (frameState, target) {
            var layerState = frameState.layerStatesArray[frameState.layerIndex];
            var pixelRatio = frameState.pixelRatio;
            var viewState = frameState.viewState;
            ...
            transform.makeScale(this.pixelTransform, 1 / pixelRatio, 1 / pixelRatio);
            transform.makeInverse(this.inversePixelTransform, this.pixelTransform);
            var canvasTransform = transform.toString(this.pixelTransform);
            ...
            if (layerState.extent) {
                var layerExtent = proj.fromUserExtent(layerState.extent, viewState.projection);
                clipped = !extent.containsExtent(layerExtent, frameState.extent) && extent.intersects(layerExtent, frameState.extent);
                if (clipped) {
    ...
    var WindLayer = /** @class */ (function (_super) {
        ...
        function WindLayer(data, options) {
            var _this = this;
        ...
        // @ts-ignore
        WindLayer.prototype.render = function (frameState, target) {
            var layerRenderer = this.getRenderer();
            if (layerRenderer.prepareFrame(frameState)) {
                return layerRenderer.renderFrame(frameState, target);
            }
        };
        WindLayer.prototype.getRenderer = function () {
            if (!this.renderer_) {
                this.renderer_ = this.createRenderer();
            }
            return this.renderer_;
        };
        WindLayer.prototype.getWrapX = function () {
            return this.get('wrapX');
        };
        WindLayer.prototype.hasRenderer = function () {
            return !!this.renderer_;
        };
        WindLayer.prototype.createRenderer = function () {
        ...
        WindLayer.prototype.updateParams = function (options) {
            if (options === void 0) { options = {}; }
            warnLog('will move to setWindOptions');
            this.setWindOptions(options);
            return this;
        };
        WindLayer.prototype.getParams = function () {
